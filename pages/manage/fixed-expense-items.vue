<template>
  <view class="page">
    <view class="hero-card">
      ...
      <text class="hero-title">维护每月固定项目</text>
      <text class="hero-desc">支持新增、编辑和删除固定支出项，后续记账页会自动同步展示。</text>
    </view>
    ...
    <view class="list-card">
      ...
      <view v-else class="empty-state">暂无固定支出项，先新增一项吧。</view>
    </view>
  </view>
</template>
...
function loadItems() {
  ...
  items.value = userData?.fixedExpenseItems || []
}
...
function handleSubmit() {
  ...
  if (editingId.value) {
    const nextList = items.value.map((item) =>
      item.id === editingId.value ? { ...item, name } : item
    )
    persistItems(nextList, "固定支出项已更新")
    return
  }
  ...
  persistItems(nextList, "固定支出项已新增")
}
...
function removeItem(item) {
  uni.showModal({
    ...
    success: ({ confirm }) => {
      ...
      persistItems(nextList, "固定支出项已删除")
    }
  })
}
